...
**Wave 1 — semantic scopes (#180).** Deliver region extents, derived containment and index, innermost/scope-path resolution, archetype validation (including exactly one declared `archetype: boss` scope), persistence checks, authoring projection, `Zone.parent_id`, and fog-safe runtime projection. Do not add a boss entity/cell, boss marker on a placed monster, boss spawning, future properties, hidden extents, or web implementation scope.
