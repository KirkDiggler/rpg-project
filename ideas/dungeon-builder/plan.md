...
3. `rpg-toolkit` compiles flat absolute extents, derives parent/index (parent = smallest strict superset; innermost = smallest containing region), and resolves locations through that scope chain. It persists only canonical authored region facts (`id`, `name`, archetype presence/value, cells) and recomputes derived state on compile/load. Zero, one, or multiple explicit archetype roles are runnable; missing conventions may be reported only through an existing surface, never a new diagnostics proto/API. It adds no boss entity/cell, placed-monster boss marker, or boss spawning.
4. `rpg-api` pins released providers and merges last after authoring/runtime wire and Load evidence.

Regions may be disconnected. Unpainted root area is valid. Painting, repainting, or deleting a region cannot alter structural floor, canonical edges, endpoint-derived door IDs, content, or content validity; removed semantics fall back. Existing room-based semantic lookups resolve scope from their established absolute location and are not renamed. Hard rejection is limited to malformed source, duplicate IDs, out-of-floor cells, incompatible non-empty `rooms:` plus `regions:`, unsupported explicit archetypes, and equal/partial overlaps; duplicate cells within one region are canonical dedupe. No lighting/audio/trigger capabilities, new boss/spawn rules, geometry/traversal, room/connector/procedural compatibility, holes/levels, lock behavior, diagnostics proto/API, or web implementation scope is included. Preserve only a generic future extension seam and location-to-scope lookup. #176–#179 remain delivered/open history and are not reopened.
...
Delivered backend history is recorded by [rpg-api-protos#206](https://github.com/KirkDiggler/rpg-api-protos/pull/206), [rpg-toolkit#881](https://github.com/KirkDiggler/rpg-toolkit/pull/881) and tags `tools/spatial/v0.6.0`, `rulebooks/dnd5e/v0.71.0`, `encounter/v0.48.0`, and [rpg-api#769](https://github.com/KirkDiggler/rpg-api/pull/769); see delivery comment [5183123668](https://github.com/KirkDiggler/rpg-project/issues/179#issuecomment-5183123668). #176–#179 remain OPEN on GitHub but are delivered history; web is independently owned. Wave 0 is LIVE VERIFIED (see [verification](https://github.com/KirkDiggler/rpg-project/issues/192#issuecomment-5206548237)); Wave 1 implementation has not started. Its validation/evidence must cover the relevant proto contract, toolkit validation and persistence, API projection, and real wire state.
