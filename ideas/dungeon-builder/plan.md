...
- `src/author/preview3d/DungeonPreview3D.tsx`: render `PutDungeonResponse.atlas` through `buildScene3D` + `SyntyHexFloor` + `AtlasWalls` + `PropModel`; camera wrapper so `SessionScene`'s `useCameraControls` is not required (orbit + the game's tactical rig).
