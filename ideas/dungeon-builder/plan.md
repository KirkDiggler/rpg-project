...
3. `rpg-toolkit` compiles flat absolute extents, derives parent/index (parent = smallest strict superset; innermost = smallest containing region), validates strict containment/overlap rules and archetypes including exactly one declared `archetype: boss` semantic scope, resolves absolute content locations through that scope chain, persists source plus derived state, and releases. It adds no boss entity/cell, placed-monster boss marker, or boss spawning.
