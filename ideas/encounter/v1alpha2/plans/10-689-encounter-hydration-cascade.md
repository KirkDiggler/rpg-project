...
Today `ToData()` is literally `return e.data` — it hands back the load-time snapshot pointer, not current held-entity state. **That changes:** with held entities that mutate (e.g. `SneakAttack.UsedThisTurn`), `ToData` cascades the held combatants' `ToData()` back into `PlayerData.DataJSON` / `MonsterData.DataJSON`. This **replaces** the host's scattered `saveAttackerConditionState` (`dnd5e_combat_resolver.go:422`) + the `publishTurnEndAndPersistReset` write-back (`end_turn.go:388-394`).

> **Implementation finding (ADR-0030, verified in code — supersedes the "dirty-gated" idea):** `IsDirty()` is set **only on HP change** (`character.go:728`, `monster.go:165`), NOT on condition mutations like `UsedThisTurn` — so dirty-gating the write-back would *silently drop* cross-RPC condition state (the subscribe-once suite caught the would-be regression). The cascade is therefore **unconditional** for now. Extending dirty-tracking to condition state — so the cascade can be efficiency-gated (Kirk's original idea) — is a tracked **future** enhancement, not #689.
