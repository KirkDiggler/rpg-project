...
- **Snapshot completeness** — what `SnapshotFor(playerID)` actually serializes. Full `PerceptionView`, plus current entity positions visible to that player, plus turn state, plus door states they've witnessed. Enumerate. Slice 1 ships a slim Snapshot (Position + RevealedHexes); a future slice grows it to "everything needed to render the player's world standalone."
...
- **Catch-up policy on reconnect** — slice 1 ships snapshot-only (load `Snapshot`, attach live stream, no event replay). Future slice adds an `EventLog` interface (in-memory for tests; Redis Streams for prod) and a replay-on-reconnect path that sends events from the player's last-seen sequence forward, then attaches the live subscription. Sequence numbers on events are already in place; the slice 1 architecture accommodates this addition without a refactor. Standard pattern: state at sequence N + events since N.
- **Combat log policy** — two valid framings: **in-character** (combat log is a UI rendering of per-player perception events; same audience filter — Alice didn't see it, it's not in her log) vs **out-of-character shared chronicle** (everyone in the encounter sees the log regardless of LoS, modeling table-level narration). Both have UX merit (in-character for immersion; out-of-character for table banter — "Steve rolled a NAT 1!"). Probably toggleable per encounter or per game. Slice 1 ships in-character only; out-of-character chronicle is additive later — broker emits to a second channel (`enc:<id>:chronicle`) alongside per-player streams without changing event types.
