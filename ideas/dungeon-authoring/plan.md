...
Acceptance for this milestone (full detail under Acceptance below): edit `internal/content/dungeons/reference-tomb.yaml`, restart the api, walk the room in the real game route — no Go touched.
...
Every rule from design.md §Validation gets a row: version==1; key/name non-empty; ≥2 rooms, unique ids; `archetype` ∈ {entrance, chamber, corridor, boss} (landed during B3's quality review, not originally in this task's own enumeration — `KnownFields` strictness catches unknown YAML *fields*, not unknown *values*, so an unrecognized archetype like `chambre` decoded, validated, and compiled cleanly before this rule existed, only dying downstream at `StartEncounter` as an `Internal` error from a different repo; implemented in `validate.go` alongside the rules below, same file, same table); connectors reference declared rooms and form a connected linear chain; exactly one `boss` archetype; `boss` entry only on the boss room; ref shape `module:type:id`; monster/boss refs resolve via `monsters.ByRef` (author-time failure); pattern ∈ {"", "empty", "scattered"}; height ≥ 4; every width ≥ 4; boss primary axis `min(boss.width, height) > 6`; counts ≥ 1; lock DC 1–30; lock ability is a known ability ref (validate against the `rulebooks/dnd5e/abilities` package the crypt constructor already imports). Structure:
...
**Path correction (as-built, verified during this task's own implementation):** content files live at `internal/content/dungeons/*.yaml`, NOT the repo-root `content/dungeons/*.yaml` this plan originally sketched — `go:embed` hard-rejects `..` patterns, so a package under `internal/content/` cannot embed a directory that lives outside its own subtree; the embed target must be a subdirectory of the package doing the embedding. `RPG_CONTENT_DIR` is unaffected — it's an unconstrained filesystem path at runtime, not subject to the embed directive's restriction, and still wins on a key collision against the embedded set.

**Files:**
- Create: `internal/content/dungeons/reference-tomb.yaml` (the delta's M1 acceptance file — a minimal entrance room + the fully-placed tomb, two rooms, honoring the ≥2-rooms/entrance-spawn constraints honestly), `internal/content/content.go` (go:embed + `RPG_CONTENT_DIR` override + `SpecByKey`), `internal/content/content_test.go`
...
func TestContentDirOverride(t *testing.T) {
	...
	// an author editing internal/content/dungeons/reference-tomb.yaml
	// locally via RPG_CONTENT_DIR must see their edit, not the stale
	// embedded copy.
}
```
...
**Pre-work, before any M2 task below starts (finding from Task N1's quality review): extract `dungeon.go`'s obstacle-placement section into `dungeon_obstacles.go`.** `encounter/dungeon.go`'s `placeRegionObstacles`/`placeVerbatimObstacles`/`regionObstacleCandidates` family (~270 lines, grown by Task N1's placed-obstacle work) is exactly the code M2's Slice C (`SeedMonsters` count-based rolling) needs to read closely and reuse candidate-pool machinery from. Splitting it out is a pure file move — no behavior change, one commit, on its own — done BEFORE M2's feature work starts so that work's diffs stay reviewable against a stable file boundary instead of landing inside an already-270-line file that just grew a fourth rejection case in M1. This mirrors the package's existing flat-file-per-concern idiom (`two_chamber.go`, `crypt_dungeon.go` are both already separate files off `dungeon.go`'s core). Commit as its own step: `refactor(encounter): extract obstacle placement into dungeon_obstacles.go (#<issue>)`, full module suite green, before Task C1 begins.
...
**Files:**
- Create: `internal/content/dungeons/crypt.yaml` (the parity file — byte-identical intent to `testdata/crypt.yaml`), `internal/content/dungeons/sunken-crypt.yaml` (the 4-room acceptance file from design.md's original v1 body, if not already shipped as part of `reference-tomb.yaml`'s evolution toward M3)
