...
- Each A/B/C/D GLB has exactly the class's three ordered idle clips plus `Walk_Forward`. For each retained non-relaxed idle, require exact clip names, target bone/channel path coverage, interpolation modes, and source keyframe time coverage. At every retained source keyframe time, evaluated pose-bone `matrix_basis` values must have a maximum absolute matrix-element delta from baseline of at most `1e-4`; record the measured maximum. `idleClips` is `Idle_Relaxed`, class-distinctive idle, `Idle_Drinking`; `Walk_Forward` is not an idle entry. Set every class weapon `bakedIntoModel` to `false` while retaining its standalone weapon file, socket fields, and weapons catalog.
...
- Semantic validation uses decoded glTF semantics, not indices: constrained baseline diff, retained-action coverage fingerprints (clip names, target bone/channel paths, interpolation, and source times), Blender-evaluated `matrix_basis` parity at `1e-4`, exact action set, exact generated A/B/C/D action parity when `swap_atlas.py` preserves actions through the same pipeline, one body mesh/no extra mesh, Root contract, baseline topology/material ownership, manifest/docs gate, and authoritative mesh/animation reports. The `1e-4` retained baseline-to-new tolerance is not raw accessor equality: normal Blender 5 round-trip drift has measured up to `6.6e-05`; GLB chunk surgery for byte-exact accessors is rejected. Name checks alone never prove weapon absence.
...
**Interfaces:** Produces `semantic_glb(gltf, bin_data) -> dict`, `retained_action_coverage(gltf, bin_data, animation) -> dict`, `validate_candidate(baseline, candidate, weapon_nodes) -> list[str]`, `validate_retained_actions(baseline, candidate, names, pose_tolerance=1e-4) -> {"errors": list[str], "maxMatrixBasisDelta": float}`, `validate_variant_parity(paths, expected) -> list[str]`, `validate_manifest_docs(baseline_manifest, candidate_manifest, baseline_readme, candidate_readme, config) -> list[str]`, and `validate_evidence_review(evidence_commit, comment_url) -> dict`. `validate_retained_actions` requires exact clip names, target bone/channel paths, interpolation modes, and source keyframe times, then reports evaluated pose-bone `matrix_basis` parity at every source keyframe time. Task 2's Blender smoke and Task 5 consume this interface.
...
- [ ] Write tests that reject material/topology/Root/mesh changes; a retained clip name, target bone/channel path, interpolation, or source-keyframe-time change; a retained evaluated `matrix_basis` mutation greater than `1e-4`; variant clip/count differences or any generated A/B/C/D decoded action-value difference; a `bakedIntoModel: true` manifest; stale baked-weapon documentation; and malformed reviewed GitHub JSON. Assert that the retained-parity result reports `maxMatrixBasisDelta`.
- [ ] Run `python3 scripts/test_character_promotion_validation.py`; expect failure before implementations exist.
- [ ] Implement semantic node/skin/mesh/material/image/accessor decoding, constrained weapon-subtree removal, retained-action coverage comparison, Blender-evaluated pose-bone `matrix_basis` comparison at every retained source keyframe time, and exact generated A/B/C/D decoded action parity. The baseline-to-new retained comparison must use `retainedPoseTolerance` (`1e-4`), never raw accessor/value equality, and report `maxMatrixBasisDelta`; the generated-variant comparison remains exact when `swap_atlas.py` preserves actions through the same pipeline. Implement exact manifest/docs rules, report-path/verdict checks, inventory SHA-256 checks, and fixture-backed review parsing. The real review lookup parses the comment ID from `EVIDENCE_REVIEW_URL`, calls `gh api repos/KirkDiggler/rpg-game-assets/issues/comments/$COMMENT_ID`, and requires the exact evidence SHA, `portraits: PASS (16 viewed)`, `animationAnatomy: PASS (64 artifacts viewed; fighter,barbarian,monk,rogue; Idle_Relaxed,Walk_Forward; 0,45,90,180)`, and signature.
...
**Interfaces:** Consumes Task 1's `validate_retained_actions(baseline, candidate, names, pose_tolerance=1e-4) -> {"errors": list[str], "maxMatrixBasisDelta": float}`. Produces `assign_armature_action(armature, action)`, `copy_named_action(source, target, name)`, `validate_export_scene(armature, body) -> dict`, exporter `--out --body --stats-out --clip`, and transfer `--target --baseline --out --body --retain`. The Blender smoke delegates retained semantics to Task 1's validator. Task 3 calls export/transfer through `stage`.

- [ ] Write a Blender-backed monk re-export smoke that transfers `Idle_Meditative` and `Idle_Drinking`, then asks Task 1's `validate_retained_actions` interface to prove exact retained coverage and matrix parity. Assert a normal Blender 5 GLB import/copy/export passes with its reported maximum at or below `1e-4` (the measured normal round-trip upper bound is `6.6e-05`), and assert a deliberate pose mutation greater than `1e-4` fails. Also assert exactly four final clips.
...
- [ ] Implement Blender 5 slot binding: clear NLA tracks, require exactly one action slot, set `armature.animation_data.action` and `action_slot`. Export only the configured target armature/body, remove only `Icosphere`, reject every additional mesh, capture source topology/material stats, import the baseline only to copy configured retained actions, remove imported baseline objects/actions, and export `Idle_Relaxed`, `Walk_Forward`, and retained clips exactly once. The smoke must evaluate the exported armature's pose-bone `matrix_basis` at every retained source keyframe time and emit the measured maximum delta through Task 1's validator.
- [ ] Re-run the command; expect PASS, the four configured clips, and a reported retained `maxMatrixBasisDelta <= 1e-4`. Commit `feat: transfer retained character actions with Blender slots`.
...
- [ ] Write tests for `stage`: it calls export/transfer for A, uses `swap_atlas.py` for B/C/D without hand-maintained geometry/actions, proves exact generated A/B/C/D decoded action parity when that pipeline preserves actions, keeps staged files repo-relative, generates mesh/animation reports, and regenerates all four portraits for a class only when inspection flags a weapon in its current portrait.
...
- [ ] Implement `validate` to fetch and verify the real review comment, run every Task 1 semantic/report/manifest/docs gate against all staged A/B/C/D files, and record each retained action's `maxMatrixBasisDelta` in metadata before writing sorted inventory hashes excluding metadata. `validate` rejects retained deltas greater than `1e-4` and generated-variant action differences where `swap_atlas.py` preserves actions through the same pipeline. Implement `apply` to require PASS metadata and exact baseline, copy only inventory paths, write metadata, and emit NUL-delimited sorted `inventory.paths UNION {metadataPath}`. Implement `verify-index` to require exactly that staged set and compare `git show :<path>` SHA-256s to inventory while separately validating metadata.
