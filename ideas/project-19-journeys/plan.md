...
- [ ] Capture `/tmp/project19-journey-pilot-before.json` from:
...
- [ ] Rename field `PVTSSF_lAHOAASbwc4Bcj4vzhXLt3s` from `Feature` to `Area` with `updateProjectV2Field`. Read back all seven existing option IDs unchanged.

- [ ] Add `Concept` to field `PVTSSF_lAHOAASbwc4Bcj4vzhXLt3w` with `updateProjectV2Field`, submitting the complete ordered option set:
...
- [ ] Create `Initiative` with the sole option `Four-player Level-3 Dungeon`:
...
- [ ] Create `Readiness` with `Shaping,Ready,Blocked` using the same command shape.

- [ ] Create or update structural labels idempotently:
...
- [ ] Verify field names are exactly `Area`, `Kind`, `Initiative`, and `Readiness`; `Feature` is absent; Kind has six options; Initiative has one; Readiness has three.
...
- [ ] Resolve live IDs and make these fields visible in every view: Title, Assignees, Status, Linked pull requests, Parent issue, Sub-issues progress, Area, Kind, Team, Initiative, Readiness.

- [ ] Create the five views and read back exact names, layouts, filters, and visible fields. The public API cannot set group-by configuration; Parent issue and Sub-issues progress are the pilot's verifiable hierarchy surface.

- [ ] Replace the Project README with a concise contract that states:
...
- [ ] Read the README and all five views back through GraphQL/`gh project view`. Stop if any filter or field is missing.
...
- [ ] Create and board `rpg-project` issue **Initiative: Four-player Level-3 Dungeon** from design §4. Label `initiative`; set Status `In Progress`, Team `Cross-team`, Area `Capstone`, and Initiative `Four-player Level-3 Dungeon`.

- [ ] Reconcile `rpg-project#169` as **Journey: Composable Dungeon Builder**:
...
- [ ] Create and board **Journey: Composable Attack Damage**:
...
- [ ] Reconcile `rpg-toolkit#979` as PR #1126's Decide slice:
...
- [ ] Link #927's still-useful context from the attack journey/#979, post a signed supersession comment, then close #927 as superseded by #1126.

- [ ] Reconcile `rpg-project#201` as **Journey: Monster Behavior in the Local Dungeon**:
...
- [ ] Point `CLAUDE.md` at `https://github.com/users/KirkDiggler/projects/19`, name `Ready Journeys` as the contributor entry, define Initiative → Journey → Slice, retain the one-issue/PR rules, and link `ideas/project-19-journeys/design.md` instead of duplicating it.

- [ ] Verify and commit the documentation amendment:
