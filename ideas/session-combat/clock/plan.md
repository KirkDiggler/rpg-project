...
- [ ] **Step 3: Sub-issue link** — #294 under journey #253 via GraphQL `addSubIssue`
  with header `GraphQL-Features: sub_issues`. *Blocked 2026-08-27 on a GraphQL
  secondary rate limit; REST covered everything else.*
- [ ] **Step 4: Project 19 fields** on #294 and each build issue — Area, Kind,
...
### Task 4: The boundary machine — `rulebooks/dnd5e/resolution`
...
- [ ] **Step 1: One step per crossing, in order, then `Done`.**
- [ ] **Step 2: The step vocabulary does NOT grow.** A crossing is "do this on the
  ...
  exists to remember.
- [ ] **Step 3: Record the tally.** `doc.go` names its own reopening trigger — *"if
  ...
  discovered later.
- [ ] **Step 4: `RefusingAnnouncer` at `resolve.go:289`**, with the comment saying why
  it is not recursion: this package never calls `EndTurn` or `form`, so no clock
  advances inside a resolution.
- [ ] **Step 5: `Cost: nil`.** A boundary has no payer. Documented so nobody later
...
### Task 5: The seam — `rulebooks/dnd5e/session`
...
- [ ] **Step 1: A struct holding `*writeScope`, not a closure over one** — so it can
  be constructed before `scope.enc` exists, exactly as `strikerSeam` documents
  (`striker.go:20-31`).
- [ ] **Step 2: NEVER adopt a new `*encounter.Encounter`.** Read `out.DirtyCharacters`
  ...
  on sheets.
- [ ] **Step 3: Name an announcer at all five load sites** — real one on the write
  path, refusing everywhere else (design.md §2 table).
- [ ] **Step 4: The proving tests**, behavioural, through a real `session.EndTurn`,
...
- [ ] **Step 5: The structural test** — no code path advances a clock without
...
### Task 6: Put it in the running game — rpg-api
...
- [ ] **Step 1: Record what is pinned now**, before touching anything.
