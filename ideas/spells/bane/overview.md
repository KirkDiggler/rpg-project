...
**Purpose:** understand the layer a mechanic joins before reading its implementation tasks.
Bane is the worked example, not the architecture's organizing principle.

**Review home:** this project PR, beside [the design](design.md) and [the plan](plan.md).
**Implementation home:** `rpg-toolkit/rulebooks/dnd5e/overview.md`, linked beside the rulebook README;
`resolution/README.md` links to it rather than copying it. Package godoc remains the authority for
exact contracts. During implementation, reconcile this proposed view with actual code and tests;
the project document retains the design record, not a second independently maintained runtime guide.

**Evidence:** existing boundaries below were inspected at toolkit `b3f899c5`.
**EXISTING** means a current responsibility, not that every legacy path has completed its transition.
**NEW/CHANGED** labels describe the Bane proposal, not shipped or tested implementation.
Diagrams show responsibilities and operation sequences, **not a Go import graph**.
...
| Thing | Lifetime / authority |
|---|---|
| Compiled definition and price | Data describing this offered operation; does not spend itself |
| Character/monster condition list | Authoritative recipient state; ordered and serialized by its owner |
| Condition's persisted blob | Rule state and provenance, not a bus, roller, or live pointer |
| Attached condition behavior | Runtime behavior reconstructed for the interaction; supplies the rule's description |
| Selected dice contribution | Unresolved operation input; has source and notation, no face |
| Resolved calculation | Settled facts: actual faces, operator, source and checked total |
| Frozen operation | Settled state needed to resume; opaque to interrupt custody and session |
...
Read the local overview before detailed task steps; then follow the owning package's contract:

- [`play/README.md`](https://github.com/KirkDiggler/rpg-toolkit/blob/main/play/README.md) — composable leaves and their numbered laws.
- [`world/README.md`](https://github.com/KirkDiggler/rpg-toolkit/blob/main/world/README.md) and [`world/doc.go`](https://github.com/KirkDiggler/rpg-toolkit/blob/main/world/doc.go) — world composition and injected rule decisions.
- [`encounter/doc.go`](https://github.com/KirkDiggler/rpg-toolkit/blob/main/rulebooks/dnd5e/encounter/doc.go) — composition and supplied capabilities.
- [`resolution/doc.go`](https://github.com/KirkDiggler/rpg-toolkit/blob/main/rulebooks/dnd5e/resolution/doc.go) — bus lifetime, data boundary, steps and payment.
- [Bane design](design.md) — approved behavior and proposed contracts; [implementation plan](plan.md) — files, dependencies and proving tests.

When this lands with code, link these package documents using repository-local paths and bind the
worked example to actual production tests. Do not copy role charters or turn this overview into a
second policy store.
