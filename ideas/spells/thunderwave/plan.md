...
- [ ] **Step 4: implement `Direct`:** copy `walkCells` (`clocks.go:908`) into `directive.go` as `directCells`, minus the active-turn assumption (call `e.mover.Move` and `e.stepTo` with the directed member, not `activeID`), plus `Cause` on each `appendMovementBeat`. Refactor `walkCells` to call the same helper with an empty cause so there is one walker. **Provokes needs a seam across three modules (found building #1658; ruled 2026-09-11):** `MovementChainEvent.OAPreventionSources` is constructed empty at `resolution/movement.go:219` and only a chain subscriber ever appends, and `Mover.Move(ctx, enc, mover, from, to)` has no room for a cause or a flag. So: (encounter, this PR) `Mover.Move` takes `MoveStep{Mover, From, To, Cause core.Ref, Forced bool}` — zero value is an ordinary walk that provokes, so no forgotten field can suppress an opportunity attack; `walkPath` passes `Forced: false`, `Direct` passes `Forced: !in.Provokes`; (resolution, PR 4) `MovementInput` gains `ForcedBy *core.Ref` that `announce` seeds into `OAPreventionSources` before publishing; (session, PR 5) `moverSeam.offerStep` (`session/mover.go:139`) passes `MoveStep.Forced/Cause` through to `resolution.NewMovement`. Skipping the Mover call for a push was rejected: it would skip the whole announcement, and with it traps, auras, and Sentinel ("a walk loads everything"). Charge no turn budget.
...
- [ ] **Step 0 (from PR 2's seam):** `MovementInput` gains `ForcedBy *core.Ref`; `announce` (`movement.go:198-219`) seeds `OAPreventionSources` with it (id = the ref, reason "forced movement") before publishing, so the existing fold drops the opportunity-attack triggers. Test: a movement with `ForcedBy` set produces no reaction trigger; without it, the existing OA test is unchanged.
- [ ] **Step 2: implement:** `const ImposedMove ImposedEffectKind = "move"`; `ImposedEffect.Move *MoveDirective` where `MoveDirective{Policy, AnchorID, Cells, Speed, Pays, Provokes}` (mirrors `CastMove` plus the anchor); a `Gather` step `imposeMove` in the same shape as `applyPreparedDamage` (`contest.go:336`): it takes a `next func(ImposedEffect) (Step, error)` continuation and hands the `ImposedMove` to it rather than appending to any list, because that is how the step chain carries effects onward. Chain it as the damage step's `next` on the failure branch (find where `applyPreparedDamage` is wired in — it is reached through the save-gate step chain, not by a direct call; grep `applyPreparedDamage(` across `resolution/*.go`). Before handing the effect on, read the target's current HP through the same `combatantFor` accessor and skip the move when it is zero ("the dropped are not pushed").
...
### Task 5.0: the mover seam carries Forced and Cause

- [ ] `session/mover.go:139` `moverSeam.offerStep`: encounter's `Mover.Move` now takes `MoveStep`; pass `MoveStep.Forced` as `MovementInput.ForcedBy` (the cause ref when forced, nil otherwise). Test: a forced step through the seam announces with the prevention source; an ordinary step does not.
...
- [ ] **Step 2: implement:** `castoutcome.go:131` gains `case resolution.ImposedMove:` → `ResultMoved`. **`ActivationResult` lives in encounter** (`activation.go:22-51` kinds, `:84-116` struct, `:336-341` closed switch in `prepareActivationResult`) — found by the session builder; the plan had guessed session. So `ResultMoved`, `Moved int`, `StoppedBy string`, and the `movedPayload` arm are one more encounter commit on PR 2 (toolkit#1658), and session re-pins before this arm. Also: because `Mover.Move`'s signature changed, task 5.0 must go FIRST or the module does not compile against the new encounter. **Ordering, ruled 2026-09-11 on the session builder's question:** `Route` is a pure computation (how far the push WILL go, what stops it); `Direct` is the walk. So (1) `route := enc.Route(...)` BEFORE `RecordCast`, no beats; (2) `RecordCast` with `ResultMoved{Moved: len(route.Path), StoppedBy: route.StoppedBy}` so the cast beat carries the cast's account and reads first — thunder, then the slide; (3) `enc.Direct(ctx, DirectInput{Route: route.Path, Cause: spell.Ref, Provokes})` AFTER `RecordCast`, so the movement beats with their cause follow in the order a client animates them. If `Direct` falls short of its route because the map changed between the two, that is its own `StoppedBy` on the movement beats, the rare case, not reconciled into the cast beat. Test: story order is [cast beat with moved/stopped_by, movement beats with cause]; the target's cell changed by exactly `len(route.Path)`.
