...
- [DR-0011 — Card-play verb](0011-card-play-verb.md) — a parallel `Encounter::playCard` reuses `strike` as the one mechanism; the card owns its behavior (`StrikeCard : Action`); the Encounter bundles `CardResult{ActionReceipt, StrikeResult}` and publishes `CardResolved`, recovering the breakdown via the `StrikeResolved` event correlated by `correlationId`. rpgkit untouched.
