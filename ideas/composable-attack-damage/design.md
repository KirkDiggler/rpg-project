...
attack-specific `ResolveDamage` stack in the integrated provider and resolution
...
Absence and repository-search guarantees in this design are scoped to the
integrated `rulebooks/dnd5e` provider and `rulebooks/dnd5e/resolution` paths.
The frozen top-level `encounter/` consumer is explicitly exempt until its
removal; it may retain legacy persisted snapshot fields while it remains on its
old provider pin.
...
Within the integrated provider and resolution paths, monster action configs use only:
...
compatibility fields in those integrated paths. In-repository monster fixtures
...
The top-level `encounter/` module is a frozen legacy consumer on an old provider
pin pending removal and is outside this integration. It does not participate in
the canonical monster-action migration or release order. Real resolution
compiles every canonical pool from hydrated action data. The standalone
scimitar action/config/loader branch is removed; goblins declare the same attack
through generic `MeleeConfig` canonical damage.
...
than mirrored in the integrated provider and resolution paths. The same change
migrates Great Weapon Fighting,
...
that stack in the integrated provider and resolution paths. `Strike` applies
damage directly once. Session adopts the returned world, records the beat, and
emits its stream event from `StrikeOutcome`.
...
4. Canonical-only JSON round trips in the integrated provider and resolution
   paths with no legacy field names.
...
22. One-handed and two-handed versatile attacks changing only the marked
    primary pool.
23. Dueling, Two-Weapon Fighting, and Rage using only canonical metadata.
24. `AttackProfile` rejecting missing, duplicate, and ability-less marker
    combinations without consuming randomness.
25. Repository searches scoped to the integrated `rulebooks/dnd5e` provider and
    `rulebooks/dnd5e/resolution` paths proving the old persisted fields,
    standalone scimitar, legacy attack APIs, compatibility mirrors, and
    obsolete callers are absent; the frozen top-level `encounter/` consumer is
    explicitly exempt until removal.
...
4. Modify and test `rulebooks/dnd5e/session` against local
...
7. Publish `rulebooks/dnd5e`; update, test, and publish
   `rulebooks/dnd5e/resolution`; then update, test, and publish
   `rulebooks/dnd5e/session` against the published D&D and resolution versions.
   Publish `rulebooks/dnd5e/encounter` only if implementation reveals an actual
